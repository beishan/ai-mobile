...
static void icon_reader(gfx_framebuffer_t *fb, int x, int y) {
    ...
    px(fb, x + 34, y + 8, 5, 12, GFX_RED);
}

static void icon_weather(gfx_framebuffer_t *fb, int x, int y) {
    px(fb, x + 9, y + 10, 12, 12, GFX_RED);
    px(fb, x + 13, y + 6, 4, 4, GFX_RED);
    px(fb, x + 4, y + 14, 4, 4, GFX_RED);
    px(fb, x + 22, y + 14, 4, 4, GFX_RED);
...
static void icon_calendar(gfx_framebuffer_t *fb, int x, int y) {
    ...
    px(fb, x + 8, y + 6, 32, 8, GFX_RED);
    px(fb, x + 14, y + 22, 5, 5, GFX_BLACK);
    px(fb, x + 24, y + 22, 5, 5, GFX_BLACK);
    px(fb, x + 34, y + 22, 5, 5, GFX_BLACK);
    px(fb, x + 24, y + 32, 8, 8, GFX_RED);
}

static void icon_game(gfx_framebuffer_t *fb, int x, int y) {
    gfx_draw_rect(fb, x + 7, y + 16, 34, 18, GFX_BLACK);
    px(fb, x + 14, y + 22, 12, 4, GFX_BLACK);
    px(fb, x + 18, y + 18, 4, 12, GFX_BLACK);
    px(fb, x + 31, y + 22, 6, 6, GFX_RED);
    px(fb, x + 27, y + 29, 6, 6, GFX_BLACK);
}
...
void ui_draw_icon(gfx_framebuffer_t *fb, ui_icon_kind_t kind, int x, int y, int selected) {
    if (selected) {
        gfx_fill_rect(fb, x - 3, y - 3, 54, 54, GFX_RED);
